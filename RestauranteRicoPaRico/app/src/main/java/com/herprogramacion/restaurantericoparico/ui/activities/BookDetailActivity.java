...
public class BookDetailActivity extends AppCompatActivity {
    ...
    public void addfav(View v){
        Book book = (Book) getIntent().getSerializableExtra(BookListActivity.BOOK_DETAIL_KEY);
        search = Find.findFavFilm(book.getOpenLibraryId());
        if (search == -1) {
            Singleton.getInstance().addToArrayFilmFav(book.getOpenLibraryId());
    ...
    public void addfav22(View v) {
        Book book = (Book) getIntent().getSerializableExtra(BookListActivity.BOOK_DETAIL_KEY);
        search = Find.findVisFilm(book.getOpenLibraryId());
        if (search == -1) {
            Singleton.getInstance().addToArrayfilmvis(book.getOpenLibraryId());
            Comidas.MoviesVis.add(new Comida(book.getOpenLibraryId(), book.getStars(), book.getTitle(), book.getImg(), book.getAuthor(), 3f, 1, 778, "03/24/2017", book.getImg()));
            Toast.makeText(this, "Serie Añadida a Favoritos", Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(this, "Se a Eliminado de Favoritos", Toast.LENGTH_SHORT).show();
    ...
    public void addfav33(View v) {
        Book book = (Book) getIntent().getSerializableExtra(BookListActivity.BOOK_DETAIL_KEY);
        search = Find.findPenFilm(book.getOpenLibraryId());
        if (search == -1) {
            Singleton.getInstance().addToArrayfilmpen(book.getOpenLibraryId());
            Comidas.MoviesPen.add(new Comida(book.getOpenLibraryId(), book.getStars(), book.getTitle(), book.getImg(), book.getAuthor(), 3f, 1, 778, "03/24/2017", book.getImg()));
            Toast.makeText(this, "Serie Añadida a Favoritos", Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(this, "Se a Eliminado de Favoritos", Toast.LENGTH_SHORT).show();
